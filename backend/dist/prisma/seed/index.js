...
async function main() {
    ...
    const tags = await Promise.all(tagNames.map(name => prisma.tag.create({ data: { name, slug: name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase() } })));
    ...
    const resource1 = await prisma.resource.create({
        data: {
            ...
            type: client_1.ResourceType.PDF,
    ...
    const resource3 = await prisma.resource.create({
        data: {
            ...
            type: client_1.ResourceType.LINK,
    ...
    await prisma.moderation.createMany({
        data: [
            { resourceId: resource1.id, moderatorId: moderator.id, status: client_1.ResourceStatus.APPROVED, reviewedAt: new Date() },
            { resourceId: resource2.id, moderatorId: moderator.id, status: client_1.ResourceStatus.APPROVED, reviewedAt: new Date() },
            { resourceId: resource3.id, moderatorId: moderator.id, status: client_1.ResourceStatus.APPROVED, reviewedAt: new Date() },
            { resourceId: resource4.id, status: client_1.ResourceStatus.PENDING, aiFlags: [] },
        ],
    });
